...
use Binshops\LaravelTicket\Models\Configuration;
...
class ConfigurationsController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('laravelticket::admin.configuration.index', compact('configurations', 'configurations_by_sections'));
    }
    ...
    {
        return view('laravelticket::admin.configuration.create');
    }
    ...
    public function store(Request $request)
    {
        ...
        \Cache::forget('laravelticket::settings'); // refresh cached settings
        return redirect()->action('\Binshops\LaravelTicket\Controllers\ConfigurationsController@index');
    }
    ...
    public function edit($id)
    {
        ...
        return view('laravelticket::admin.configuration.edit', compact('configuration', 'should_serialize', 'default_serialized'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        if ($request->serialize) {
            ...
            if (!Auth::attempt($request->only('password'), false, false)) {
                return back()->withErrors([trans('laravelticket::admin.config-edit-auth-failed')]);
            }
            if (false === eval('$value = serialize('.$value.');')) {
                return back()->withErrors([trans('laravelticket::admin.config-edit-eval-error')]);
            }
        }

        $configuration->update(['value' => $value, 'lang' => $request->lang]);

        Session::flash('configuration', trans('laravelticket::lang.configuration-name-has-been-modified', ['name' => $request->name]));
        // refresh cached settings
        \Cache::forget('laravelticket::settings');
        \Cache::forget('laravelticket::settings.'.$configuration->slug);
        //return redirect(route('laravelticket::admin.configuration.index'));
        return redirect()->action('\Binshops\LaravelTicket\Controllers\ConfigurationsController@index');
    }
}
