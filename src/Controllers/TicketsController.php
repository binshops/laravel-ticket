...
use Binshops\LaravelTicket\Models;
use Binshops\LaravelTicket\Models\Agent;
use Binshops\LaravelTicket\Models\Category;
...
class TicketsController extends Controller
{
    ...
    {
        $this->middleware('Binshops\LaravelTicket\Middleware\ResAccessMiddleware', ['only' => ['show']]);
        $this->middleware('Binshops\LaravelTicket\Middleware\IsAgentMiddleware', ['only' => ['edit', 'update']]);
        $this->middleware('Binshops\LaravelTicket\Middleware\IsAdminMiddleware', ['only' => ['destroy']]);
    ...
    public function data($complete = false)
    {
        ...
        $collection
            ->join('users', 'users.id', '=', 'laravelticket.user_id')
            ->join('laravelticket_statuses', 'laravelticket_statuses.id', '=', 'laravelticket.status_id')
            ->join('laravelticket_priorities', 'laravelticket_priorities.id', '=', 'laravelticket.priority_id')
            ->join('laravelticket_categories', 'laravelticket_categories.id', '=', 'laravelticket.category_id')
            ->select([
                'laravelticket.id',
                'laravelticket.subject AS subject',
                'laravelticket_statuses.name AS status',
                'laravelticket_statuses.color AS color_status',
                'laravelticket_priorities.color AS color_priority',
                'laravelticket_categories.color AS color_category',
                'laravelticket.id AS agent',
                'laravelticket.updated_at AS updated_at',
                'laravelticket_priorities.name AS priority',
                'users.name AS owner',
                'laravelticket.agent_id',
                'laravelticket_categories.name AS category',
            ]);
    ...
    public function index()
    {
        ...
        return view('laravelticket::index', compact('complete'));
    }
    ...
    protected function PCS()
    {
        ...
        $categories = Cache::remember('laravelticket::categories', $time, function () {
    ...
    public function create()
    {
        ...
        return view('laravelticket::tickets.create', compact('priorities', 'categories'));
    }
    ...
    public function store(Request $request)
    {
        $this->validate($request, [
            ...
            'priority_id' => 'required|exists:laravelticket_priorities,id',
            'category_id' => 'required|exists:laravelticket_categories,id',
        ]);
        ...
        session()->flash('status', trans('laravelticket::lang.the-ticket-has-been-created'));
    ...
    public function show($id)
    {
        ...
        return view('laravelticket::tickets.show',
    ...
    public function update(Request $request, $id)
    {
        $this->validate($request, [
            ...
            'priority_id' => 'required|exists:laravelticket_priorities,id',
            'category_id' => 'required|exists:laravelticket_categories,id',
            'status_id'   => 'required|exists:laravelticket_statuses,id',
            'agent_id'    => 'required',
        ]);
        ...
        session()->flash('status', trans('laravelticket::lang.the-ticket-has-been-modified'));
    ...
    public function destroy($id)
    {
        ...
        session()->flash('status', trans('laravelticket::lang.the-ticket-has-been-deleted', ['name' => $subject]));
    ...
    public function complete($id)
    {
        if ($this->permToClose($id) == 'yes') {
            ...
            session()->flash('status', trans('laravelticket::lang.the-ticket-has-been-completed', ['name' => $subject]));

            return redirect()->route(Setting::grab('main_route').'.index');
        }

        return redirect()->route(Setting::grab('main_route').'.index')
            ->with('warning', trans('laravelticket::lang.you-are-not-permitted-to-do-this'));
    }
    ...
    public function reopen($id)
    {
        if ($this->permToReopen($id) == 'yes') {
            ...
            session()->flash('status', trans('laravelticket::lang.the-ticket-has-been-reopened', ['name' => $subject]));
