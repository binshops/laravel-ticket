...
class LaravelTicketTableSeeder extends Seeder
{
    ...
    public function run()
    {
        ...
        foreach ($this->statuses as $name => $color) {
            $status = \Binshops\LaravelTicket\Models\Status::create([
                'name'  => $name,
                'color' => $color,
            ]);
        }
        ...
        foreach ($this->categories as $name => $color) {
            $category = \Binshops\LaravelTicket\Models\Category::create([
                'name'  => $name,
                'color' => $color,
            ]);
        ...
        foreach ($this->priorities as $name => $color) {
            $priority = \Binshops\LaravelTicket\Models\Priority::create([
                'name'  => $name,
                'color' => $color,
            ]);
        }
        $categories_qty = \Binshops\LaravelTicket\Models\Category::count();
        $priorities_qty = \Binshops\LaravelTicket\Models\Priority::count();
        $statuses_qty = \Binshops\LaravelTicket\Models\Status::count();
        ...
        for ($u = 1; $u <= $this->users_qty; $u++) {
            ...
            for ($t = 1; $t <= $tickets_qty; $t++) {
                ...
                $category = \Binshops\LaravelTicket\Models\Category::find($rand_category);
                ...
                $ticket = new \Binshops\LaravelTicket\Models\Ticket();
                ...
                for ($c = 1; $c <= $comments_qty; $c++) {
                    ...
                    $comment = new \Binshops\LaravelTicket\Models\Comment();
